...
class Mp3dDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', num_points=20000,
        ...
        self.data_path = os.path.join("/checkpoint/zaiweizhang/votenet-data/mp3d/", 'matterport_train_detection_data')
        tempdict = {}
        templist = []
        for x in os.listdir(self.data_path):
            tempname = os.path.basename(x).split("_")[0]
            if tempname not in tempdict:
                tempdict[tempname] = 1
            else:
                tempdict[tempname] += 1
        ...
        all_scan_names = list(set(filterlist))
        print (len(list(set(templist))))
        print (len(set(all_scan_names)))
        if split_set=='all':            
            self.scan_names = all_scan_names
        elif split_set in ['train', 'val', 'test']:
            split_filenames = os.path.join(ROOT_DIR, 'mp3d/meta_data',
                'mp3d_{}.txt'.format(split_set))
            with open(split_filenames, 'r') as f:
                temp_scan_names = f.read().splitlines()   
            # remove unavailiable scans
            self.scan_names = [sname for sname in all_scan_names \
                if sname.split(".")[0] in temp_scan_names]
...
if __name__=='__main__': 
    dset = Mp3dDetectionDataset(use_height=True, num_points=40000)
