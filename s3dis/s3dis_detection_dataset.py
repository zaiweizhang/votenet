...
import pc_util
from model_util_s3dis import rotate_aligned_boxes

from model_util_s3dis import S3disDatasetConfig
DC = S3disDatasetConfig()
MAX_NUM_OBJ = 256
MEAN_COLOR_RGB = np.array([109.8, 97.2, 83.8])

class S3disDetectionDataset(Dataset):
       
    def __init__(self, split_set='train', num_points=20000,
                 use_color=False, use_height=False, augment=False, scan_idx_list="None"):

        self.data_path = os.path.join("/checkpoint/zaiweizhang/votenet-data/s3dis/", 'ours_new/')
        all_scan_names = []
        for x in os.listdir(self.data_path):
            data = os.path.basename(x).split("_")
            temps = ""
            for i in range(4):
                temps += data[i]+"_"
            all_scan_names.append(temps)
        all_scan_names = list(set(all_scan_names))
        if split_set=='all':            
            ...
            else:#if split_set == 'val':
                self.scan_names = []
                for scan in all_scan_names:
                    if "Area_5" in scan:
                        self.scan_names.append(scan)
            if scan_idx_list != "None":
                scan_idx_list = np.load(scan_idx_list)
    ...
    def __getitem__(self, idx):
        ...
        mesh_vertices = np.load(os.path.join(self.data_path, scan_name)+'pt.npy')
        data = np.load(os.path.join(self.data_path, scan_name)+'all_noangle.npy')
        instance_labels = data[:,-2]#np.load(os.path.join(self.data_path, scan_name)+'_ins_label.npy')
        semantic_labels = data[:,-1]
        instance_bboxes = []
        for obj in np.unique(instance_labels):
            temp = data[instance_labels==obj,:][0,[0,1,2,3,4,5,8]]
            if temp[-1] in DC.nyu40ids:
                instance_bboxes.append(temp)
...
if __name__=='__main__': 
    dset = S3disDetectionDataset(use_height=True, num_points=40000)
